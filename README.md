...
- `viewer/index.html` - страница локального BVH viewer (Three.js).
- `viewer/js/main.js` - основная логика viewer (загрузка BVH/модели, ретаргет, playback).
- `viewer/models/en_0.vrm` - дефолтная модель для viewer (подгружается автоматически).
...
Улучшение OpenCV-анализа (сложный свет/шум/дальний план):

```bash
OPENCV_ENHANCE=light MAX_FRAME_SIDE=1280 ./convert.sh --auto think.mp4 --all
```

Агрессивнее для дальнего/мелкого человека в кадре:

```bash
OPENCV_ENHANCE=light MAX_FRAME_SIDE=1280 ROI_CROP=auto ./convert.sh --auto think.mp4 --all
```
...
- `--opencv-enhance {off,light,strong}` (по умолчанию `off`, OpenCV preprocessing перед MediaPipe)
- `--max-frame-side` (по умолчанию `0`, если >0 — кадр сжимается до указанной максимальной стороны)
- `--roi-crop {off,auto}` (по умолчанию `off`, адаптивный ROI-кроп вокруг человека между кадрами)
...
Режимы OpenCV preprocessing:

- `off`: без доп. обработки (быстрее, базовый вариант).
- `light`: мягкое подавление шума + CLAHE + лёгкое осветление; обычно лучший старт для реальных видео.
- `strong`: более агрессивная обработка для очень тёмных/шумных роликов.

Режим ROI-кропа:

- `off`: детекция по полному кадру.
- `auto`: после первого успешного кадра детекция идёт по динамическому кропу вокруг человека, при промахе делается fallback на полный кадр.

Пример прямого запуска с улучшением:

```bash
.venv/bin/python convert_video_to_bvh.py \
  --input think.mp4 \
  --output-bvh output/think.bvh \
  --opencv-enhance light \
  --max-frame-side 1280 \
  --roi-crop auto
```
...
- автозагрузка дефолтной модели `viewer/models/en_0.vrm`,
